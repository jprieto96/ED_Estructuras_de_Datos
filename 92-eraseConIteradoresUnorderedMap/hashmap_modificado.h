...
public:
    // constructor
    unordered_map(int n = TAM_INICIAL, Hash h = Hash(), Pred p = Pred()) : array(n, nullptr), nelems(0), hash(h), pred(p) {}
...
protected:
    ...
    protected:
        ...
        Iterador(pointer_tabla t, Clave const &c) : tabla(t)
        {
            ...
            Link ant;
            act = tabla->array[ind];
            if (!tabla->localizar(c, ant, act))
            { // se devuelve iterador al final
                act = nullptr;
                ind = tabla->array.size();
            }
        }
...
public:
    ...
    iterator erase(iterator const& it) {
        if(it.act == nullptr)
            throw std::out_of_range("El iterador no apunta a ningun par clave-valor.");

        iterator nuevo_it = it;
        ++nuevo_it; // Iterador a devolver que apunta al siguiente elemento al eliminado de la tabla
        Link temp = array[it.ind];
        Link ant = nullptr;
        // Recorremos la lista de nodos que hay en el cajon que indica el indice del iterador
        // hasta que encontramos el nodo a eliminar y nos guardamos su anterior
        while(temp != it.act) {
            // Si el siguiente nodo es el que queremos eliminar nos guardamos el anterior para despues poder enlazar bien los nodos al hacer la eliminacion
            if(temp->sig == it.act)
                ant = temp;
            temp = temp->sig;
        }
